...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await inventoryRepository.approveStockOpname({
    ...
    if ("error" in result) {
      throw new ApiError("CONFLICT", "Stock opname hanya bisa diapprove setelah semua hasil fisik disubmit", 409);
    }
